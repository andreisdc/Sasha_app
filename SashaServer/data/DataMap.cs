       ...
       public Property? GetPropertyById(Guid propertyId)
{
    ...
    return new Property
    {
        ...
        // ================================
        // 📸 PROPERTY PHOTOS - METODE OPTIMIZATE
        // ================================

        // ✅ Obține cover photo pentru o proprietate specifică
    
        // ✅ Obține toate cover photos pentru o listă de proprietăți (optimizat pentru liste)
       

       
        // ✅ Actualizează cover photo pentru o proprietate
        public bool UpdatePropertyCoverPhoto(Guid propertyId, Guid newCoverPhotoId)
        {
            using var conn = new NpgsqlConnection(_connectionString);
            conn.Open();

            // Începe o tranzacție pentru a asigura consistența
            using var transaction = conn.BeginTransaction();

            try
            {
                // 1. Setează toate pozele ca non-cover pentru această proprietate
                var resetCmd = new NpgsqlCommand(
                    @"UPDATE t_property_photos 
            SET is_cover = false 
            WHERE property_id = @property_id",
                    conn);

                resetCmd.Parameters.AddWithValue("property_id", propertyId);
                resetCmd.ExecuteNonQuery();

                // 2. Setează noua poză ca cover
                var setCmd = new NpgsqlCommand(
                    @"UPDATE t_property_photos 
            SET is_cover = true 
            WHERE id = @photo_id AND property_id = @property_id",
                    conn);

                setCmd.Parameters.AddWithValue("photo_id", newCoverPhotoId);
                setCmd.Parameters.AddWithValue("property_id", propertyId);
                var rowsAffected = setCmd.ExecuteNonQuery();
        ...
        // ✅ Obține numărul de poze pentru o proprietat
...
// ✅ Obține cover photo pentru o proprietate specifică
public PropertyPhoto? GetPropertyCoverPhoto(Guid propertyId)
{
    using var conn = new NpgsqlConnection(_connectionString);
    conn.Open();
    
    var cmd = new NpgsqlCommand(
        @"SELECT id, property_id, file_path, is_cover, created_at 
        FROM t_property_photos 
        WHERE property_id = @property_id AND is_cover = true
        LIMIT 1", 
        conn);
...
// ✅ Obține toate cover photos pentru o listă de proprietăți (optimizat pentru liste)
public Dictionary<Guid, PropertyPhoto> GetPropertiesCoverPhotos(List<Guid> propertyIds)
{
    var coverPhotos = new Dictionary<Guid, PropertyPhoto>();
    ...
    var cmd = new NpgsqlCommand(
        @"SELECT id, property_id, file_path, is_cover, created_at 
        FROM t_property_photos 
        WHERE property_id = ANY(@property_ids) AND is_cover = true", 
        conn);
    
    cmd.Parameters.AddWithValue("property_ids", propertyIds.ToArray());
    ...
    while (reader.Read())
    {
        ...
        coverPhotos[photo.PropertyId] = photo;
    }
    
    return coverPhotos;
}

// ✅ Obține prima poză a unei proprietăți (dacă nu există cover)
public PropertyPhoto? GetPropertyFirstPhoto(Guid propertyId)
{
    using var conn = new NpgsqlConnection(_connectionString);
    conn.Open();
...
// ✅ Obține toate pozele pentru o proprietate (pentru detalii)
public List<PropertyPhoto> GetPropertyPhotosWithDetails(Guid propertyId)
{
    var list = new List<PropertyPhoto>();
...
// ✅ Șterge toate pozele unei proprietăți
public bool DeleteAllPropertyPhotos(Guid propertyId)
{
    using var conn = new NpgsqlConnection(_connectionString);
    conn.Open();
    
    var cmd = new NpgsqlCommand(
        "DELETE FROM t_property_photos WHERE property_id = @property_id", 
        conn);
...
// ✅ Obține numărul de poze pentru o proprietate
public int GetPropertyPhotosCount(Guid propertyId)
{
    using var conn = new NpgsqlConnection(_connectionString);
    conn.Open();
    
    var cmd = new NpgsqlCommand(
        "SELECT COUNT(*) FROM t_property_photos WHERE property_id = @property_id", 
        conn);
...
// ✅ Verifică dacă o proprietate are poze
public bool PropertyHasPhotos(Guid propertyId)
{
    using var conn = new NpgsqlConnection(_connectionString);
    conn.Open();
    
    var cmd = new NpgsqlCommand(
        "SELECT EXISTS(SELECT 1 FROM t_property_photos WHERE property_id = @property_id)", 
        conn);
...
// ================================
// 📋 GET PROPERTIES OPTIMIZAT CU COVER PHOTOS
// ================================

// ✅ Metodă optimizată pentru a obține proprietăți cu cover photos
public List<PropertyWithCover> GetPropertiesWithCoverPhotos()
{
    var properties = new List<PropertyWithCover>();
    
    using var conn = new NpgsqlConnection(_connectionString);
    conn.Open();

    // Query care join-ează proprietățile cu cover photos
    var cmd = new NpgsqlCommand(
        @"SELECT 
            p.id, p.owner_id, p.title, p.description, p.location_type, 
            p.address, p.city, p.county, p.country, p.postal_code,
            p.latitude, p.longitude, p.status, p.price_per_night,
            p.min_nights, p.max_nights, p.check_in_time, p.check_out_time,
            p.max_guests, p.bathrooms, p.kitchen, p.living_space,
            p.pet_friendly, p.smoke_detector, p.fire_extinguisher,
            p.carbon_monoxide_detector, p.lock_type, p.average_rating,
            p.review_count, p.neighborhood_description, p.tags,
            p.instant_book, p.is_verified, p.created_at, p.updated_at,
            pp.file_path as cover_image_url,
            pp.id as cover_photo_id
        FROM t_properties p
        LEFT JOIN t_property_photos pp ON p.id = pp.property_id AND pp.is_cover = true
    ...
    while (reader.Read())
    {
        var property = new PropertyWithCover
        {
            Id = reader.GetGuid(0),
            OwnerId = reader.GetGuid(1),
            Title = reader.GetString(2),
            ...
            Tags = reader.IsDBNull(30) ? Array.Empty<string>() : reader.GetFieldValue<string[]>(30),
            InstantBook = reader.GetBoolean(31),
            IsVerified = reader.GetBoolean(32),
            CreatedAt = reader.GetDateTime(33),
            UpdatedAt = reader.GetDateTime(34),
            CoverImageUrl = reader.IsDBNull(35) ? null : reader.GetString(35),
            CoverPhotoId = reader.IsDBNull(36) ? null : reader.GetGuid(36)
        };
...
// ✅ Metodă pentru a obține proprietățile utilizatorului cu cover photos
public List<PropertyWithCover> GetUserPropertiesWithCoverPhotos(Guid userId)
{
    var properties = new List<PropertyWithCover>();
...
// ✅ Clasa pentru proprietăți cu cover photo
public class PropertyWithCover : Property
{
